...
def fewshot_sampler(dataset, num_shots):
    """
    从 ImageFolder 类型 dataset 中每类采样 num_shots 个样本。
    返回：torch.utils.data.Subset
    """
    ...
    for label, indices in label_to_indices.items():
        if len(indices) < num_shots:
            print(f"⚠️ Warning: Class {label} only has {len(indices)} samples in this domain.")
        sampled = random.sample(indices, min(num_shots, len(indices)))
        selected.extend(sampled)

    return Subset(dataset, selected)


def get_lodo_dataloaders(dataset, test_env, num_shots=5, batch_size=32, preprocess_fn=None):
    """
    从一个 MultipleEnvironmentImageFolder（如 OfficeHome）实例中，
    返回 leave-one-domain-out 的训练集和验证集 DataLoader。

    - dataset: OfficeHome 实例
    - test_env: 留出作为验证的子域编号
    - num_shots: 每类 few-shot 数量（0 表示 zero-shot）
    - preprocess_fn: 预处理函数（如 CLIPWrapper.get_preprocess()）
    """
    train_sets = []
    val_set = []
    for i in range(len(dataset)):
        dataset[i].transform = preprocess_fn
        if i != test_env:
            if num_shots > 0:
                few = fewshot_sampler(dataset[i], num_shots)
